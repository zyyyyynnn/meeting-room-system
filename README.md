...
- 会议室管理：列表查看对已登录用户开放，状态、容量、设备与维护信息治理由超级管理员执行
- 我的预约：查看、取消、删除个人预约
- 预约审批：管理员审批待处理预约，超级管理员可撤销历史审批结果
- 用户管理：管理员与超级管理员管理账号、角色与启停状态
...
| `SUPER_ADMIN` | 超级管理员，具备管理员全部能力，并额外执行会议室高级治理与审批撤销 | `/dashboard` |
...
课程实验交付物已整理到 [docs](docs) 目录，按“文档 / 图示 / 项目页面截图”分类管理：

- [docs/文档](docs/文档)：6 份正式 `.docx` 文档
- [docs/图示](docs/图示)：架构图、流程图、ER 图、时序图、用户故事地图
- [docs/项目页面截图](docs/项目页面截图)：产品原型对应的系统页面截图

正式文档包括：

- `docs/文档/01-需求规格说明书.docx`：需求背景、角色权限、功能需求、业务规则和非功能需求
- `docs/文档/02-产品原型设计.docx`：基于当前系统真实页面截图整理的 6 个核心页面原型
- `docs/文档/03-用户故事地图.docx`：当前版本用户故事地图与故事清单
- `docs/文档/04-系统架构设计文档.docx`：系统总体架构、认证鉴权流程和预约时序设计
- `docs/文档/05-数据库设计文档.docx`：MySQL 三张核心表与 Redis 运行态设计
- `docs/文档/06-API接口文档.docx`：认证、会议室、预约、审批、统计、用户管理接口说明

图示与截图直接来源于当前项目实现和页面运行结果，文档内容与当前代码版本保持一致。
